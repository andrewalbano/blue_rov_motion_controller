...
class MotionControl:
    ...
    def publish_velocity_setpoints(self):
        ...
        self.velocity_setpoint.linear.z = self.vx_setpoint
        ...
        if controller.invoked:
            ...
            if controller.velocity_setpoint_testing:
                ...
                if controller.hold_pose:
                    ...
                    rospy.loginfo_throttle(10,f"Holding postion at (x,y,z): {controller.hold_pose_waypoint.position.x:.2f}, {controller.hold_pose_waypoint.position.y:.2f},{controller.hold_pose_waypoint.position.z:.2f}")
                else:
                    ...
                    if controller.reached_waypoint():
                        ...
                        if controller.waypoint_index < controller.num_waypoints-1:
                            rospy.loginfo(f"Reached waypoint {controller.waypoint_index +1}: {controller.current_waypoint.pose.position.x:.2f}, {controller.current_waypoint.pose.position.y:.2f}, {controller.current_waypoint.pose.position.z:.2f}")
                            ...
                            rospy.loginfo_throttle(15,f"Reached the last waypoint, holding postion at waypoint {controller.waypoint_index +1}: {controller.current_waypoint.pose.position.x:.2f}, {controller.current_waypoint.pose.position.y:.2f}, {controller.current_waypoint.pose.position.z:.2f}")
