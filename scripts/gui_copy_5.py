...
class WaypointGui:  
    def __init__(self, master):
        ...
        # Jack added this Sat. March 8th, 2025
        self.map_boundaries = Button(self.button_frame, text="Map Boundaries", command=self.dvl_map_boundaries)
        self.make_square.grid(row=row_index, column=column_index, columnspan=column_span, sticky="ew",  padx=2, pady=2)
        row_index +=1
        ...
        # Jack added this: Sat. March 8th, 2025
        # Need both barometer data and DVL data for true depth projection:
        # self.sub2 = rospy.Subscibrer('/') 
    ...
    # Jack added this: Sat. March 8th, 2025
    def scan_map_boundaries(self):
        # While doing this, collect z data and visulize
        self.add_defined_waypoint(x=0,y=0,z=0,roll=np.pi/2,pitch=0,yaw =0)
        self.add_defined_waypoint(x=0,y=0,z=0,roll=np.pi/2,pitch=np.pi,yaw =0)
        self.add_defined_waypoint(x=0,y=0,z=0,roll=np.pi/2,pitch=2*np.pi,yaw =0)
        # Go home after
        rospy.loginfo("Publishing DVL sweep waypoints")
        self.pub3.publish(self.goal_waypoints)
        self.pub5.publish(self.desired_path)
