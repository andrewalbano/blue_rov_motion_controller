...
from nav_msgs.msg import OccupancyGrid as grid
...
class SonarTopic():
    def __init__(self):
        ...
        self.map = rospy.Subscriber('/map', grid, self.grid_callback)
    ...
    def grid_callback(self,msg:grid):
        self.grid = grid()
        self.grid = msg
    ...
    def cylindrical2cartesian(self):
        ...
        thresh = 100
        mask = transform > thresh
        transform = np.multiply(mask.astype(float),transform)
