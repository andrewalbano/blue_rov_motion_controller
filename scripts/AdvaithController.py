...
def create_semicircle(center,radius,angle,increments):
    ...
    yaw = np.arctan2(POSE[1,:], POSE[0,:]) + np.pi
    print(POSE.shape, yaw.shape)
    POSE = np.concatenate((POSE, yaw[None, :]), axis=0)
    return POSE.T
...
current_position = {'x': 0.0, 'y': 0.0, 'yaw': 0.0, "depth": 0.0}
...
def calculate_control(current_position, waypoint):
    ...
    # # Radius vector to orbit center:
    # CoR_x = CenterofROT[0] - current_position['x']
    # CoR_y = CenterofROT[1] - current_position['y']
    ...
    # des_yaw_orbit = np.arctan2(CoR_y,CoR_x)

    yaw_error = waypoint[2] - current_position['yaw']
...
def sonar_image_callback(msg:Image):
    global sonar_picture
    sonar_picture = msg.data
...
def listener():
    ...
    rospy.Subscriber('/BlueROV/depth', Float64, depth_callback)
