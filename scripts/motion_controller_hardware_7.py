...
from std_msgs.msg import Bool, Int8, Float32MultiArray, tuple
import time
...
class MapBuilder(SonarTopic,MotionControl):
    ...
    def local2FF(self):
        ...
        XY_FF2ROV = np.array([self.state['x'], self.state['y']])  # (1,2)
        ...
        if controller.invoked:
           # Build state machine here:
            ...
            if controller.send_signal == "manual":
                # The controller must be invoked to use sonar data
                Map = MapBuilder_0()

                # We access the sonar data here ^^
                # Now we need to build a map function which does the following:
                # 1. Interprets the 2D image_array object into MHL walls
                # 2. Translates the wall positions from the local, moving frame to the fixed frame defined at start of controller
                # 3. Defines safe-fly boundary zone
