...
class MotionControl:
    ...
    def manual_velocity_setpoint_controller(self):
        # controlling only for velocity based on user defined setpoint sending signals based on manual mode
...
def manual_velocity_setpoint_controller(controller, master):
    # controlling only for velocity based on user defined setpoint sending signals based on manual mode
...
def manual_PWM_user_controlled(controller, master):
    # receives input from user interface for signal, replicates joystick mode
    rospy.loginfo_throttle(10,"PWM defined by user")
    # publishing information fo plotting
    controller.publish_velocity_setpoints()
    controller.publish_pwm_commands()

    # sends actual control
    send_control_manual(controller.x_pwm,controller.y_pwm, controller.z_pwm, controller.yaw_pwm, master)
...
def manual_control_signal(controller, master):

    # controlling only for velocity based on user defined setpoint
    if controller.velocity_setpoint_testing:
        manual_velocity_setpoint_controller(controller, master)
    ...
    while not rospy.is_shutdown():
        ...
        if controller.invoked:
            ...
            if controller.send_signal == "manual":
                ...
                # sending a joystick or pwm signal from the gui
                elif controller.pwm_testing:
                    rospy.loginfo_throttle(10,"PWM defined by user")
                    # publishing information fo plotting
                    controller.publish_velocity_setpoints()
